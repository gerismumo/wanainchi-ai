...
export class UsersRepository {
 ...
 async findTotalUsers(): Promise<number> {
  const result:any = await this.knex("users").count("* as count").first();
  ...
  async createUser(data: {
    ...
    const { first_name, last_name, email, password_hash, role_id, country } =
      data;
  ...
  async updateUserWithRoles(userId: string, data: UpdateUserDto) {
    const { first_name, last_name, phone_number, email, is_active, roles, avatar_url } =
      data;

    return this.knex.transaction(async (trx) => {
      // 1. update user fields (NO password update)
  ...
  private baseUserWithRolesQuery() {
    return this.knex('users as u')
      ...
      .select(
        ...
        'u.country',
        'u.added_by',
        'u.can_sell_below_net_price',
        'u.min_sale_price_percentage',
        'u.below_net_price_approved_by',
        'u.below_net_price_approved_at',
        'u.below_net_price_notes',
        'u.created_at',
        this.knex.raw(`
        COALESCE(
          json_agg(
